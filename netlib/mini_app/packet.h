...
// Introduce this to support << and >> for long string
...
template <class T>
inline const sox::Unpack& operator >> (const sox::Unpack& p, std::vector<T>& cc)
{
...
template <class T>
inline const sox::Unpack& operator >> (const sox::Unpack& p, std::list<T>& cc)
{
	unmarshal_container(p, std::back_inserter(cc));
	return p;
}
...
template <class T1, class T2>
inline const sox::Unpack& operator >> (const sox::Unpack& p, std::map<T1, T2>& cc)
{
...
template <class T1, class T2>
inline const sox::Unpack& operator >> (const sox::Unpack& p, std::list<std::map<T1, T2> >& cc)
{
...
// 新增协议请从这里继承，并在pack_me&unpack_me里面打包解包。
struct CallidBase : public sox::Marshallable
{
	std::string callid;
	...
	virtual void pack_me(Pack & pk) const = 0;
	virtual void unpack_me(const Unpack & up) = 0;
};
